...
ENV VIRTUAL_ENV /opt/esperoj-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/esperoj/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/esperoj/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/esperoj/ && touch src/esperoj/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/esperoj/
...
ENTRYPOINT ["/opt/esperoj-env/bin/esperoj"]
CMD []
